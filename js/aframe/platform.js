...
//      squeakDisplay.unfreeze = null}
  ...
  if (!(mobilecheck())) {
	...
	scene.render()
//	if (!squeakDisplay.unfreeze) squeakDisplay.unfreeze = SqueakJS.vm.freeze()
...
function forwardProjectedMouseEvents(camera, plane, canvas) {
  ...
  document.addEventListener(
    'wheel',
    function (event) {
      if (!squeakDisplay.vm && !canvas.otherCanvasActive) {
	if (event.deltaY > 0) {
  ...
  // Move around in XZ in response to the left-hand thumbstick.
  ...
  // Move around in Z/yaw in response to the right-hand thumbstick.
...
//	squeakDisplay.unfreeze = SqueakJS.vm.freeze()
      }})}
